...
// Column names 
$fields = array('Store Name','Transaction',  'Product SKU', 'Quantity','Price','Total','Date'); 
...
$sql = "SELECT a.*,b.store_name FROM posrev.admin_daily_transaction_details a 
...
".$filter_store." ".$filter_product." ".$filter_transaction_type." ";
...
while($row = mysqli_fetch_array($result)){
    ...
    $report_arr[] = array("id" => $row['details_id'],"store_name" => $row['store_name'], "name" => $row['product_sku'], 
                        ...
                        ,'price'=>$row['price'],'total'=>$row['total'],'date'=>$row['zreading']);
...
for($i = 0; $i < count($report_arr); $i++){
    ...
    'total'=>$report_arr[$i]['total'],'date'=>$report_arr[$i]['date']); 
...
inner join posrev.admin_daily_transaction c on c.transaction_number = a.transaction_number
WHERE DATE_FORMAT(a.zreading, '%Y-%m-%d') >= '".$Date1."' AND DATE_FORMAT(a.zreading, '%Y-%m-%d') <= '".$Date2."' 
".$filter_store." ".$filter_product." ".$filter_transaction_type." group by c.transaction_number";
